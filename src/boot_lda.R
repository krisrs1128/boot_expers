...
for (i in seq_len(n_replicates)) {
    ...
    stan_data <- list(
       ...
       "gamma" = rep(gamma, nrow(theta_hat))
    )
    ...
    beta_hat <- posterior_mean(vb_fit$beta, c("v", "k"))
    theta_hat <- posterior_mean(vb_fit$theta, c("i", "k"))

    write_feather(beta_hat, paste0(output_dir, "beta-boot-", fit_id, batch_id, i, ".feather"))
    write_feather(theta_hat, paste0(output_dir, "theta-boot-", fit_id, batch_id, i, ".feather"))
}
